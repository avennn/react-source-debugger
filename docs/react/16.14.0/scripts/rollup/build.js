...
const __EXPERIMENTAL__ =
  typeof RELEASE_CHANNEL === 'string' ? RELEASE_CHANNEL === 'experimental' : true;
...
const requestedBundleTypes = argv.type ? parseRequestedNames([argv.type], 'uppercase') : [];
...
function getBabelConfig(updateBabelOptions, bundleType, packageName, externals, isDevelopment) {
  const canAccessReactObject = packageName === 'react' || externals.indexOf('react') !== -1;
  let options = {
...
function shouldSkipBundle(bundle, bundleType) {
  ...
  if (requestedBundleNames.length > 0) {
    const isAskingForDifferentNames = requestedBundleNames.every(
      ...
      // search.
      (requestedName) => (bundle.entry + '/index.js').indexOf(requestedName) === -1
    );
...
async function createBundle(bundle, bundleType) {
  ...
  const isFBRNBundle =
    bundleType === RN_FB_DEV || bundleType === RN_FB_PROD || bundleType === RN_FB_PROFILING;
  ...
  const mainOutputPath = Packaging.getBundleOutputPath(bundleType, filename, packageName);
