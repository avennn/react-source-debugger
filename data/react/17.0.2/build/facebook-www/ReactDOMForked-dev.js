...
var debugRenderPhaseSideEffectsForStrictMode = dynamicFeatureFlags.debugRenderPhaseSideEffectsForStrictMode,
...
var SHOULD_NOT_DEFER_CLICK_FOR_FB_SUPPORT_MODE = IS_LEGACY_FB_SUPPORT_MODE | IS_REPLAYED | IS_CAPTURE_PHASE; // We do not want to defer if the event system has already been
...
function extractEvents$3(dispatchQueue, domEventName, targetInst, nativeEvent, nativeEventTarget, eventSystemFlags, targetContainer) {
  if (!enableEagerRootListeners) {
    ...
    domEventName !== 'selectionchange' && !eventListenerSet.has('onSelect') && !eventListenerSet.has('onSelectCapture')) {
      return;
    }
  }
...
function ensureListeningTo(rootContainerInstance, reactPropEvent, targetElement) {
  if (!enableEagerRootListeners) {
    ...
    var rootContainerElement = rootContainerInstance.nodeType === COMMENT_NODE ? rootContainerInstance.parentNode : rootContainerInstance;

    {
      if (rootContainerElement == null || rootContainerElement.nodeType !== ELEMENT_NODE && // This is to support rendering into a ShadowRoot:
      rootContainerElement.nodeType !== DOCUMENT_FRAGMENT_NODE) {
        error('ensureListeningTo(): received a container that was not an element node. ' + 'This is likely a bug in React. Please file an issue.');
      }
    }
...
function markForceUpdateScheduled(fiber, lane) {
  {
    if (supportsUserTiming) {
      ...
      performance.mark("--schedule-forced-update-" + formatLanes$1(lane) + "-" + componentName);
    }
  }
}
function markStateUpdateScheduled(fiber, lane) {
  {
    if (supportsUserTiming) {
      ...
      performance.mark("--schedule-state-update-" + formatLanes$1(lane) + "-" + componentName);
    }
  }
}
...
function bailoutHooks(current, workInProgress, lanes) {
  ...
  if ( enableDoubleInvokingEffects) {
    workInProgress.flags &= ~(MountPassiveDev | Passive | MountLayoutDev | Update);
  } else {
...
function mountImperativeHandle(ref, create, deps) {
  ...
  if ( enableDoubleInvokingEffects) {
    return mountEffectImpl(MountLayoutDev | Update, Layout, imperativeHandleEffect.bind(null, create, ref), effectDeps);
  } else {
...
function mountOpaqueIdentifier() {
  ...
  if (getIsHydrating()) {
    ...
    if ((currentlyRenderingFiber$1.mode & BlockingMode) === NoMode) {
      if ( enableDoubleInvokingEffects) {
        currentlyRenderingFiber$1.flags |= MountPassiveDev | Passive | PassiveStatic;
      } else {
          ...
          if (primarySubtreeFlags !== NoFlags) {
            {
              ...
              invokeGuardedCallback(null, recursivelyCommitLayoutEffects, null, child, finishedRoot);
          ...
          {
            {
              ...
              invokeGuardedCallback(null, commitLayoutEffectsForProfiler, null, finishedWork, finishedRoot);

              if (hasCaughtError()) {
                var _error = clearCaughtError();

                captureCommitPhaseError(finishedWork, finishedWork.return, _error);
              }
          ...
          if (_primarySubtreeFlags !== NoFlags) {
            {
              ...
              invokeGuardedCallback(null, recursivelyCommitLayoutEffects, null, _child, finishedRoot);
  ...
  (executionContext & DiscreteEventContext) !== NoContext && schedulerPriority === UserBlockingPriority) {
    ...
    if (decoupleUpdatePriorityFromScheduler) {
      ...
      if (schedulerLanePriority !== currentUpdateLanePriority && currentUpdateLanePriority !== NoLanePriority) {
        {
          error('Expected current scheduler lane priority %s to match current update lane priority %s', schedulerLanePriority, currentUpdateLanePriority);
        }
      }
    }
...
function commitRootImpl(root, renderPriorityLevel) {
  ...
  if (subtreeHasEffects || rootHasEffect) {
    ...
    {
      setCurrentFiber(finishedWork);
      invokeGuardedCallback(null, recursivelyCommitLayoutEffects, null, finishedWork, root);
...
function warnIfUnmockedScheduler(fiber) {
  {
    if (didWarnAboutUnmockedScheduler === false && Scheduler.unstable_flushAllWithoutAsserting === undefined) {
      if (fiber.mode & BlockingMode || fiber.mode & ConcurrentMode) {
        ...
        error('Starting from React v18, the "scheduler" module will need to be mocked ' + 'to guarantee consistent behaviour across tests and browsers. ' + 'For example, with jest: \n' + // Break up requires to avoid accidentally parsing them as dependencies.
        "jest.mock('scheduler', () => require" + "('scheduler/unstable_mock'));\n\n" + 'For more info, visit https://reactjs.org/link/mock-scheduler');
      }
    }
  }
...
function registerEventOnNearestTargetContainer(targetFiber, domEventName, isCapturePhaseListener, targetElement) {
  if (!enableEagerRootListeners) {
    ...
    if (targetContainer === null) {
      {
        error('ReactDOM.createEventHandle: setListener called on an target ' + 'that did not have a corresponding root. This is likely a bug in React.');
      }
...
function registerReactDOMEvent(target, domEventName, isCapturePhaseListener) {
  ...
  if (target.nodeType === ELEMENT_NODE) {
    if (!enableEagerRootListeners) {
      ...
      if (targetFiber === null) {
        {
          error('ReactDOM.createEventHandle: setListener called on an element ' + 'target that is not managed by React. Ensure React rendered the DOM element.');
        }
    ...
    if (!enableEagerRootListeners) {
      ...
      registerEventOnNearestTargetContainer(_targetFiber, domEventName, isCapturePhaseListener, null);
    }
...
//# sourceMappingURL=ReactDOMForked-dev.js.map
