...
const enableProfilerCommitHooks = true; // Logs additional User Timing API marks for use with an experimental profiling tool.
...
const continuousReplayableEvents = ['dragenter', 'dragleave', 'focusin', 'focusout', 'mouseover', 'mouseout', 'pointerover', 'pointerout', 'gotpointercapture', 'lostpointercapture'];
...
function extractEvents$3(dispatchQueue, domEventName, targetInst, nativeEvent, nativeEventTarget, eventSystemFlags, targetContainer) {
  if (!enableEagerRootListeners) {
    const eventListenerSet = getEventListenerSet(targetContainer); // Track whether all listeners exists for this plugin. If none exist, we do
...
function listenToReactEvent(reactEvent, rootContainerElement, targetElement) {
  if (!enableEagerRootListeners) {
    ...
    const dependenciesLength = dependencies.length; // If the dependencies length is 1, that means we're not using a polyfill
    ...
    if (isPolyfillEventPlugin) {
      const listenerSet = getEventListenerSet(rootContainerElement); // When eager listeners are off, this Set has a dual purpose: it both
      ...
      const isCapturePhaseListener = reactEvent.substr(-7) === 'Capture' && // Edge case: onGotPointerCapture and onLostPointerCapture
...
function ensureListeningTo(rootContainerInstance, reactPropEvent, targetElement) {
  if (!enableEagerRootListeners) {
    ...
    const rootContainerElement = rootContainerInstance.nodeType === COMMENT_NODE ? rootContainerInstance.parentNode : rootContainerInstance;
...

{
  ...
  updateHostComponent$1 = function (current, workInProgress, type, newProps, rootContainerInstance) {
    ...
    const updatePayload = prepareUpdate(instance, type, oldProps, newProps, rootContainerInstance); // TODO: Type this specific to this type of component.
...
function createRootImpl(container, tag, options) {
  ...
  if (enableEagerRootListeners) {
    ...
    if (hydrate && tag !== LegacyRoot) {
      const doc = containerNodeType === DOCUMENT_NODE ? container : container.ownerDocument; // We need to cast this because Flow doesn't work
...
function getNearestRootOrPortalContainer(node) {
  while (node !== null) {
    const tag = node.tag; // Once we encounter a host container or root container
...
function registerReactDOMEvent(target, domEventName, isCapturePhaseListener) {
  ...
  if (target.nodeType === ELEMENT_NODE) {
    if (!enableEagerRootListeners) {
      const targetElement = target; // Check if the DOM element is managed by React.
